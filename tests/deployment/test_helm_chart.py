...
@requires_helm
@pytest.mark.fail_slow("30s")
def test_root_path_env_absent_by_default() -> None:
    deploy = _by_kind(_render(), "Deployment")[0]
    container = deploy["spec"]["template"]["spec"]["containers"][0]
    assert "RELAY_ROOT_PATH" not in {e["name"] for e in container["env"]}


@requires_helm
@pytest.mark.fail_slow("30s")
def test_root_path_env_rendered_when_set() -> None:
    deploy = _by_kind(_render("config.rootPath=/relay"), "Deployment")[0]
    container = deploy["spec"]["template"]["spec"]["containers"][0]
    env = {e["name"]: e.get("value") for e in container["env"]}
    assert env["RELAY_ROOT_PATH"] == "/relay"
    # Probes hit the pod directly, so they stay unprefixed (the relay tolerates bare paths).
    assert container["livenessProbe"]["httpGet"]["path"] == "/liveness"
    assert container["readinessProbe"]["httpGet"]["path"] == "/readiness"
...
def test_pii_keyring_secret_is_create_if_absent() -> None:
    ...
    # The keyring source is a single base64(32-byte) master key (no epoch wrapper).
    key_b64 = base64.b64decode(secret["data"]["keyring.json"]).decode()
