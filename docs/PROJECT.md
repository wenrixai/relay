...
verification is mandatory and not configurable — there is no per-channel field and no `RELAY_*`
setting that disables it, and a channel `tls` block is rejected as an unknown field. Full reference in
